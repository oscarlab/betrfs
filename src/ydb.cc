...
env_fs_poller(void *arg) {
    ...
    //printf("\n func:%s, line:%d, path = %s, r = %d, errno = %d\n", __func__, __LINE__, env->i->dir, r, ftfs_get_errno());
    assert(r == 0);
...
env_get_engine_status (DB_ENV * env, TOKU_ENGINE_STATUS_ROW engstat, uint64_t maxrows,  uint64_t *num_rows, fs_redzone_state* redzone_state, uint64_t * env_panicp, char * env_panic_string_buf, int env_panic_string_length, toku_engine_status_include_type include_flags) {
    ...
    if ( !(env)     ||
        ...
        *env_panicp = env->i->is_panicked;
        ...
        {
            /* wkj: remove stack allocation */
            YDB_LAYER_STATUS XMALLOC(ydb_stat);
        ...
        {
            /* wkj: remove stack allocation */
            YDB_C_LAYER_STATUS XMALLOC(ydb_c_stat);
            ...
            toku_free(ydb_c_stat);
        }
        {
            /* wkj: remove stack allocation */
            YDB_WRITE_LAYER_STATUS XMALLOC(ydb_write_stat);
            ...
            toku_free(ydb_write_stat);
        }
        {
            /* wkj: remove stack allocation */
            LE_STATUS XMALLOC(lestat);                    // Rice's vampire
        ...
        {
            /* wkj: remove stack allocation */
            CHECKPOINT_STATUS XMALLOC(cpstat);
        ...
        {
            /* wkj: remove stack allocation */
            CACHETABLE_STATUS XMALLOC(ctstat);
	...
	{
            FT_FLUSHER_STATUS XMALLOC(flusherstat);
            ...
            toku_free(flusherstat);
        }
        {
            /* wkj: remove stack allocation */
            FT_HOT_STATUS XMALLOC(hotstat);
        ...
        {
            /* wkj: remove stack allocation */
            LOGGER_STATUS XMALLOC(loggerstat);
            ...
            toku_free(loggerstat);
        }

        {
            /* wkj: remove stack allocation */
            INDEXER_STATUS XMALLOC(indexerstat);
...
env_get_engine_status_text(DB_ENV * env, char * buff, int bufsiz) {
    ...
    char * panicstring = (char *) toku_xmalloc(sizeof(char) * stringsize);
    ...
    if (r) {
        ...
        for (uint64_t row = 0; row < num_rows; row++) {
            ...
            case UNIXTIME:
                {
                    //char tbuf[26];
                    char * tbuf = (char *) toku_xmalloc(sizeof(char)*26);
                    memset(tbuf, 0, 26);
