...
/* toku_txn_commit recursively commits all the txns which may breaks the kernel stack,
 * use the non-recursive iteration version instead -JYM */
...
#else  /* defined TOKU_LINUX_MODULE */
typedef enum marker {
    ENTRY,  //entering the stack frame
    BACKTRACK,  //backtracking to upper level
    EXIT, //finished recursion
} marker;

typedef union stack_elem {
    void * addr;
    marker status;
    int ret_val;
} stack_elem;

typedef struct txn_commit_args {
    DB_TXN * txn;
    uint32_t flags;
    TXN_PROGRESS_POLL_FUNCTION poll;
    void *poll_extra;
    bool release_mo_lock;
    bool low_priority;
} txn_commit_args;

static inline txn_commit_args * txn_commit_args_alloc_init(DB_TXN * txn, uint32_t flags, TXN_PROGRESS_POLL_FUNCTION poll, void *poll_extra, bool release_mo_lock, bool low_priority) {
    txn_commit_args * XMALLOC(cur_args);
    cur_args -> txn = txn;
    cur_args -> flags = flags;
    cur_args -> poll = poll;
    cur_args -> poll_extra = poll_extra;
    cur_args -> release_mo_lock = release_mo_lock;
    cur_args -> low_priority = low_priority;
    return cur_args;
}
...
static int toku_txn_commit(DB_TXN * txn, uint32_t flags,
    ...
    stack.push(e1);
    stack_elem e2 = {.addr= cur_args};
    stack.push(e2);
    while(!stack.is_empty()) {
        switch(status) {
            case ENTRY: {
                cur_args = (txn_commit_args *)stack.pop().addr;
                DB_TXN * cur_txn = cur_args->txn;
                uint32_t cur_flags = cur_args->flags;
                HANDLE_PANICKED_ENV(cur_txn->mgrp);
                if (db_txn_struct_i(cur_txn)->child) {
                    stack_elem e3 = {.addr = (void*)cur_args};
                    stack.push(e3);

                    stack_elem e4 = {.status = BACKTRACK};
                    stack.push(e4);

                    txn_commit_args *child_args =
                        txn_commit_args_alloc_init(db_txn_struct_i(cur_txn)->child, cur_flags,
                                                   NULL, NULL, false, false);

                    stack_elem e5 =  {.addr = (void*)child_args};
                    stack.push(e5);
                    status = marker::ENTRY; //recursion.
                } else { //base case
                    status = stack.pop().status;
                    TXN_PROGRESS_POLL_FUNCTION cur_poll = cur_args->poll;
                    void * cur_poll_extra = cur_args->poll_extra;
                    bool cur_release_mo_lock = cur_args->release_mo_lock;
                    bool cur_low_priority = cur_args->low_priority;
                    int r = base_nonrecursive_case(cur_txn, cur_flags,
                                                   cur_poll,cur_poll_extra,
                                                   cur_release_mo_lock,
                                                   cur_low_priority);
                    stack_elem e6 = {.ret_val = r};
                    stack.push(e6);
                    toku_free(cur_args);
                }
                break;
            }
            case BACKTRACK: {//execute the base case and return;
                ret = stack.pop().ret_val;
                cur_args = (txn_commit_args *)stack.pop().addr;
                status = stack.pop().status;

                DB_TXN * cur_txn = cur_args->txn;
                if (ret !=0 && !toku_env_is_panicked(cur_txn->mgrp)) {
                    env_panic(txn->mgrp, ret, "Recursive child commit failed "
                              "during parent commit.\n");
                }
                HANDLE_PANICKED_ENV(cur_txn->mgrp);
                ...
                bool cur_release_mo_lock = cur_args->release_mo_lock;
                bool cur_low_priority = cur_args->low_priority;
                int r = base_nonrecursive_case(cur_txn, cur_flags, cur_poll,
                                               cur_poll_extra,
                                               cur_release_mo_lock,
                                               cur_low_priority);
