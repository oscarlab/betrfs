...
static int
le_cursor_callback(ITEMLEN keylen, bytevec key, ITEMLEN UU(vallen), bytevec val, void *extra, bool lock_only, bool UU(is_indirect)) {
...
build_index(DB_INDEXER *indexer) {
    ...
    if ( result == 0 ) {
        ...
        toku_checkpoint(cp, env->i->logger, NULL, NULL, NULL, NULL, INDEXER_CHECKPOINT, false);
