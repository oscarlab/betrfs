...
static DB_TXN *txn;
static uint32_t find_num;

static long closemode = -1; // must be set to 0 or 1 on command line
static long logsize   = -2; // must be set to a number from -1 to 20 inclusive, on command line.
...
init(void) {
    ...
    r=db->open(db, null_txn, TOKU_TEST_DATA_DB_NAME, 0, DB_BTREE, DB_CREATE, S_IRWXU|S_IRWXG|S_IRWXO);
    CKERR(r);
...
extern "C" int test_test_abort4(void);
int test_test_abort4(void) {
    pre_setup();
    closemode =0;
    logsize = -1;
    runtests();
