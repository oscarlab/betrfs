...
static bool top_is_delete;
static uint8_t junkvalbuf[MAX_SIZE];
static DBT junkval;
static uint8_t valbufs[MAX_NEST][MAX_SIZE];
static DBT vals        [MAX_NEST];
static uint8_t keybuf [MAX_SIZE];
static DBT key;
static uint8_t types  [MAX_NEST];
static uint8_t currval[MAX_NEST];
static DB_TXN   *txns   [MAX_NEST];
static DB_TXN   *txn_query;
static DB_TXN   *patient_txn;
//static int which_expected;
...
extern "C" int test_txn_nested5(void);
int test_txn_nested5(void) {
    pre_setup();
    initialize_values();
    int i;
    setup_db();
