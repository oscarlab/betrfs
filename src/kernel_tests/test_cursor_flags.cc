...
test_cursor_flags (int cursor_flags, int expectr) {
    ...
    r=toku_fs_reset(TOKU_TEST_FILENAME_DATA, S_IRWXU+S_IRWXG+S_IRWXO);
    assert(r==0);

    DB_ENV *env;
    r = db_env_create(&env, 0); assert(r == 0);
    r = env->open(env, TOKU_TEST_FILENAME_DATA, DB_CREATE+DB_PRIVATE+DB_INIT_MPOOL+DB_INIT_LOG+DB_INIT_TXN, 0); assert(r == 0);
    r = env->txn_begin(env, NULL, &null_txn, 0);
