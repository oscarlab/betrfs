...
static int order[NUM+1];
static uint32_t length[NUM];
static uint8_t data[NUM][MAX_LENGTH];
static DBT descriptors[NUM];
static DB_ENV *env;

enum {NUM_DBS=2};
static DB *dbs[NUM_DBS];
static DB_TXN *txn = NULL;
static DB_TXN *null_txn;
static int last_open_descriptor = -1;

static int abort_type;
static int get_table_lock;
static uint64_t num_called = 0;
...
extern "C" int test_test_db_descriptor(void);
int test_test_db_descriptor(void) {
