...
static void run_test(bool is_resetting, bool prelock) {
    ...
    IN_TXN_COMMIT(env, NULL, txn_1, 0, {
            ...
            { int chk_r = db->open(db, txn_1, TOKU_TEST_ENV_DIR_NAME, NULL, DB_BTREE, DB_CREATE, 0666); CKERR(chk_r); }
        });
    if (prelock) {
...
extern "C" int test_test_update_broadcast_with_empty_table(void);
int test_test_update_broadcast_with_empty_table(void) {
    pre_setup();
    setup();
    run_test(true,true);
    run_test(false,true);
