...
run_test(void) {
    ...
    r = env->txn_begin(env, 0, &txn, 0); CKERR(r);
    char *key = (char *)toku_xmalloc(100 * sizeof *key);
    char *val = (char *)toku_xmalloc(val_size * sizeof *val);
    ...
    for (t = 0; t<100; t++) {
        ...
        for (uint64_t i=nrows; i>0; i--) {
            //char key[100];
            memset(key, 0, 100);
            snprintf(key, 100, "%08llu", (unsigned long long)2*(i-1)+1);
...
*/
extern "C" int test_keyrange_merge(void);
