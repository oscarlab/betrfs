...
test_env (const char *envdir0, const char *envdir1, int expect_open_return) {
    ...
    int envflags = DB_INIT_LOCK | DB_INIT_LOG | DB_INIT_MPOOL | DB_INIT_TXN | DB_CREATE | DB_PRIVATE | DB_RECOVER;
...
test_logdir (const char *envdir0, const char *datadir0, const char *envdir1, const char *datadir1, int expect_open_return) {
    ...
    /* Bug is here disable it temporarily */
    if (expect_open_return != 0) {
        int envflags0 = DB_INIT_LOCK | DB_INIT_MPOOL | DB_CREATE | DB_PRIVATE | DB_RECOVER;
        r = env2->open(env2, envdir1, envflags0, S_IRWXU+S_IRWXG+S_IRWXO);
        INT(r);
        DBG;
        CKERR(r);
    }
...
extern "C" int test_multiprocess(void);
int test_multiprocess(void) {
    ...
    char *env1 = (char *)toku_xmalloc((TOKU_PATH_MAX+1) * sizeof *env1);
