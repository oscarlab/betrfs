...
static uint64_t num_pivots_fetched_prefetch;
static uint64_t num_basements_decompressed_aggressive;
static uint64_t num_basements_decompressed_prefetch;
static uint64_t num_basements_fetched_aggressive;
static uint64_t num_basements_fetched_prefetch;
...
test_bulk_fetch (uint64_t n, bool prelock, bool disable_prefetching) {
    ...
    //uint64_t keys[n];
    uint64_t * keys = (uint64_t *) toku_xmalloc(sizeof(uint64_t)*n); 
    uint64_t i;
    for (i=0; i<n; i++) {
        keys[i] = i;
    }
...
extern "C" int test_test_bulk_fetch(void);
int test_test_bulk_fetch(void) {
    pre_setup();
    test_bulk_fetch(10000, false, true);

    num_pivots_fetched_prefetch = 0;
    num_basements_decompressed_aggressive = 0;
    num_basements_decompressed_prefetch = 0;
    num_basements_fetched_aggressive = 0;
    num_basements_fetched_prefetch = 0;
