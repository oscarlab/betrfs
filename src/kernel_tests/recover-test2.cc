...
static void internal_recover_test2(bool do_commit, bool do_recover_committed) {
    srandom(0xDEADBEEF);
    for (i=0; i<N; i++) {
	//char ks[100];  
	...
	//char vs[1000]; 
        ...
        memset(vs,0,1000);
        snprintf(vs, 1000,"v%d.%0*d", i, (int)(sizeof(char)*1000-100), i);
...
extern "C" int test_recover_test2(void);
int test_recover_test2(void) {
    pre_setup();
    internal_recover_test2(true,false);
    internal_recover_test2(false,true);
    post_teardown();
    return 0;
}
