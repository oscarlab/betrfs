...
static DB_ENV *null_env = NULL;
static DB *null_db = NULL;
static DB_TXN *null_txn = NULL;
static DBC *null_cursor = NULL;
...
static void root_fifo_41(int n, int ntxn, bool do_populate) {
    ...
    int i;
    for (i=0; i<ntxn; i++) {
        r = env->txn_begin(env, null_txn, &txn[i], 0);
        assert(r == 0);
        assert(txn[i] != NULL);
    }
...
extern "C" int test_root_fifo_41(void);
int test_root_fifo_41(void) {
    int i;
    int n = -1;
    int ntxn = -1;
