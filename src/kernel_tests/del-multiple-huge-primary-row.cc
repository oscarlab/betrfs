...
verify_del_multiple(DB_ENV *env, DB *db[], int ndbs, int nrows) {
    ...
    for (int i = 0; i < nrows; i++) {
        ...
        //int v[max(ndbs,1024)]; 
        int n_v = max(ndbs, 1024); 
        int * v = (int *) toku_xmalloc(n_v* sizeof(int));
        get_data(v, i, ndbs);
        DBT pri_data; dbt_init(&pri_data, &v[0], n_v*sizeof(int));
...
populate_primary(DB_ENV *env, DB *db, int ndbs, int nrows) {
    ...
    for (int i = 0; i < nrows; i++) {
        ...
        int n_v = max(ndbs, 1024);
        int * v = (int *) toku_xmalloc(n_v*sizeof(int));
        memset(v, 0, n_v*sizeof(int)); 
        get_data(v, i, ndbs);
        
        DBT key; dbt_init(&key, &k, sizeof k);
        DBT val; dbt_init(&val, &v[0], n_v*sizeof(int));
...
run_test(int ndbs, int nrows) {
    ...
    // YZJ: BUG ON more than 2 DBs 
    assert(ndbs == 2);
    const char* dbname[ndbs];
...
extern "C" int test_del_multiple_huge_primary_row(void);
int test_del_multiple_huge_primary_row(void) {
    int r;
    int ndbs = 2;
    int nrows = 2;
    pre_setup();
