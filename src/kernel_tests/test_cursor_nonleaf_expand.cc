...
static DBC *
new_cursor (DB *db, DB_TXN *txn, int k, int v, int op) {
    DBC *cursor;
    int r;
...
test_cursor_nonleaf_expand (int n, int reverse) {
    ...
    DBC *cursor0 = new_cursor(db, txn0, htonl(0), 0, DB_FIRST); assert(cursor0);
    r = db_put(db, htonl(n), n); assert(r == 0);

    r = env->txn_begin(env, NULL, &txnn, 0); assert_zero(r);
    DBC *cursorn = new_cursor(db, txnn, htonl(n), n, DB_LAST); assert(cursorn);
    ...
    r = txnn->commit(txnn, 0); assert_zero(r);
...
extern "C" int test_test_cursor_nonleaf_expand(void);
int test_test_cursor_nonleaf_expand(void) {
