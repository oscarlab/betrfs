...
static void run_test(void) {
    ...
    IN_TXN_COMMIT(env, NULL, txn_indexer, 0, {
        // create DB
            ...
            { int chk_r = hot_index_db->open(hot_index_db, txn_indexer, TOKU_TEST_META_DB_NAME, NULL, DB_BTREE, DB_CREATE|DB_IS_HOT_INDEX, 0666); CKERR(chk_r); }
...
extern "C" int test_update_broadcast_indexer(void);
int test_update_broadcast_indexer(void) {
    pre_setup();
    setup();
    run_test();
    cleanup();
