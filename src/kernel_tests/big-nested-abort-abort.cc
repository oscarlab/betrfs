...
static void insert (int i, int j) {
    char hello[ARRSZ], there[ARRSZ];
    DBT key,data;
    if (verbose) printf("Insert %d\n", i);
    snprintf(hello, ARRSZ, "hello%d", i);
    snprintf(there, ARRSZ, "there%d", j);
...
static void lookup (int i, int expect, int expectj) {
    ...
    if (expect==0) {
	CKERR(r);
	snprintf(there, ARRSZ, "there%d", expectj);
...
extern "C" int test_big_nested_abort_abort(void);
int test_big_nested_abort_abort(void) {
    pre_setup();
    setup();
    printf("setup\n");
    test_abort_abort();
