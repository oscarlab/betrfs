...
static uint32_t kbuf[MAX_DBS][MAX_KEY/4];
static uint32_t vbuf[MAX_DBS][MAX_VAL/4];
static DBT dest_keys[MAX_DBS];
static DBT dest_vals[MAX_DBS];
...
extern "C" int test_env_put_multiple(void);
int test_env_put_multiple(void) {
    pre_setup();
    uint32_t which;
