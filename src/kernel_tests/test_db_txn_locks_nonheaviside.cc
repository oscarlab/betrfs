...
setup_dbs (void) {
    ...
    init_txn('\0');
    r = db->open(db, txns[(int)'\0'], TOKU_TEST_DATA_DB_NAME, NULL, DB_BTREE, DB_CREATE, 0600);
        CKERR(r);
    commit_txn('\0');
...
extern "C" int test_test_db_txn_locks_nonheaviside(void);
int test_test_db_txn_locks_nonheaviside(void) {
    pre_setup();	
    test();
	/*
	  test_abort(0);
