...
#ifndef __TEST_H
#define __TEST_H
#define IS_TDB 1 //note this should be fixed in CMakefile
#define USE_TDB 1
/*
...
//    char panic_string[panic_string_len];
    char * panic_string = (char *) toku_xmalloc(sizeof(char) * panic_string_len);
...
env_update_multiple_test_no_array(
    ...
    DBT_ARRAY * key_arrays = (DBT_ARRAY*) toku_xmalloc(sizeof(DBT_ARRAY) * num_keys);
    DBT_ARRAY * val_arrays = (DBT_ARRAY*) toku_xmalloc(sizeof(DBT_ARRAY) * num_vals);
    ...
    toku_free(key_arrays);
    toku_free(val_arrays);
    return r;
}
    ...
    uint32_t *flags_array)
{
    ...
    DBT_ARRAY * key_arrays = (DBT_ARRAY*) toku_xmalloc(sizeof(DBT_ARRAY) * num_dbs);
    DBT_ARRAY * val_arrays = (DBT_ARRAY*) toku_xmalloc(sizeof(DBT_ARRAY) * num_dbs);
...
static inline void pre_setup(void) {
#if IS_TDB
    int rinit = toku_ft_layer_init_with_panicenv();
    CKERR(rinit);
#endif
#if !IS_TDB && DB_VERSION_MINOR==4 && DB_VERSION_MINOR == 7
    db_env_set_func_malloc(toku_malloc);
...
static inline void post_teardown(void) {
#if IS_TDB
    toku_ydb_destroy();
#endif
