...
static int
do_fsync (int fd) {
    //dprintf(STDERR, "%8.6fs Thread %ld start fsyncing\n", get_tdiff(), pthread_self());
    inc_fsync_count();
    int r = fsync(fd);
    //dprintf(STDERR, "%8.6fs Thread %ld done  fsyncing\n", get_tdiff(), pthread_self());
    return r;
}
...
printtdiff (int N) {
    ...
    uint64_t diff = toku_tdiff(&thistime, &prevtime);
    // diff has units of microseconds
    if (verbose) {
#ifdef TOKU_LINUX_MODULE
        // difference between this printf and the one below: this one doesn't use floating point
        // since floating point is not enabled in the kernel by default. If we enable floating
        // point in the kernel, we should be able to remove this ifdef, and use the latter printf.
        printf("test_groupcommit_count: %10lu us. %4d fsyncs for %4d threads %s %8lu tps, %8lu tps/thread\n",
            diff,
            fcount-prev_count,
            N,
            do_sync ? "with sync         " : "with DB_TXN_NOSYNC",
            NITER*N*1000000/diff,
            NITER*1000000/diff);
#else
        printf("test_groupcommit_count: %10lu us. %4d fsyncs for %4d threads %s %8.1f tps, %8.1f tps/thread\n",
            diff,
            fcount-prev_count,
            N,
            do_sync ? "with sync         " : "with DB_TXN_NOSYNC",
            NITER*N*1000000.0/diff,
            NITER*1000000.0/diff);
#endif
    }
...
#ifdef TOKUDB
static int log_max_n_threads_over_10 = 3;
#else
// BDB is slow.  Reduce the work.
static int log_max_n_threads_over_10 = 2;
#endif


extern "C" int test_test_groupcommit_count(void);
int test_test_groupcommit_count(void) {
    do_sync = 1;
    verbose = 1;
    exit_one = 0;
    fsync_count_maybe_lockprotected=0;
    pre_setup();
