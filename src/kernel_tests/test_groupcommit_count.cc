...
printtdiff (int N) {
    ...
    if (verbose)
        printf("you can't use floating point in the kernel buddy. fix that (N=%d)\n", N);
        /*printf("%s: %10.6fs %4d fsyncs for %4d threads %s %8.1f tps, %8.1f tps/thread\n", 
//progname, 
                        "test_groupcommit_count",
diff, fcount-prev_count,
			N,
