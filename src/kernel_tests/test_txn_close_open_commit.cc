...
test_txn_close_open_commit (void) {
    ...
    r = db_create(&db, env, 0); assert(r == 0);
    r = db->open(db, txn, TOKU_TEST_DATA_DB_NAME, 0, DB_UNKNOWN, 0, S_IRWXU+S_IRWXG+S_IRWXO); assert(r == 0);
...
extern "C" int test_test_txn_close_open_commit(void);
int test_test_txn_close_open_commit(void) {
    pre_setup();
