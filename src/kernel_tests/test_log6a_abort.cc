...
static void make_db (void) {
    ...
    r=pending_i->open(pending_i, tid, TOKU_TEST_SFS_ONE_NAME, 0, DB_BTREE, DB_CREATE, S_IRWXU+S_IRWXG+S_IRWXO); CKERR(r);
    r=pending_d->open(pending_d, tid, TOKU_TEST_SFS_TWO_NAME, 0, DB_BTREE, DB_CREATE, S_IRWXU+S_IRWXG+S_IRWXO); CKERR(r);
    r=committed->open(committed, tid, TOKU_TEST_SFS_THREE_NAME, 0, DB_BTREE, DB_CREATE, S_IRWXU+S_IRWXG+S_IRWXO); CKERR(r);
...
int test_test_log6a_abort(void) {
    ...
    memset(peni_data, 0, N*sizeof(struct pair));
    make_db();
    post_teardown();
    return 0;
}
