...
insert_n (uint32_t ah, int datasize) {
    ...
    uint64_t *vdata = (uint64_t *)toku_xmalloc(datasize * sizeof(*vdata));
    ...
    dbt_init(&key, &an, 4);
    DBT val;
    dbt_init(&val, vdata, sizeof(vdata)*datasize);
...
static void
test_filesize_work (bool sequential)
{
    int N=1<<14;
    int r, i, sizeM;
...
extern "C" int test_filesize(void);
int test_filesize(void) {
	pre_setup();    
	setup();
	if (verbose) print_engine_status(env);
	test_filesize_work(true);
	if (verbose) {
