...
static const unsigned int NUM_KEYS = (1<<17);
static const unsigned int MAGIC_EXTRA = 0x4ac0ffee;

static const char original_data[] = "original: ha.rpbkasrkcabkshtabksraghpkars3cbkarpcpktkpbarkca.hpbtkvaekragptknbnsaotbknotbkaontekhba";
static const char updated_data[]  = "updated: crkphi30bi8a9hpckbrap.k98a.pkrh3miachpk0[alr3s4nmubrp8.9girhp,bgoekhrl,nurbperk8ochk,bktoe";
...
static int run_test(bool shutdown_before_update, bool shutdown_before_verify) {
    ...
    if (shutdown_before_update) {
        ...
        IN_TXN_COMMIT(env, NULL, txn_reopen, 0, {
                { int chk_r = db->open(db, txn_reopen, TOKU_TEST_DATA_DB_NAME, NULL, DB_BTREE, DB_CREATE, 0666); CKERR(chk_r); }
            });
    }
...
extern "C" int test_test_large_update_broadcast_small_cachetable(void);
int test_test_large_update_broadcast_small_cachetable(void) {
