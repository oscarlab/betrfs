...
static void check_callback(const DBT *end_key, uint64_t actually_skipped, void *extra) {
    ...
    int real_start_key = min(max(e->start_key, 0), num_keys);
    int expected_key = min(real_start_key + (e->skip_len / (2 * sizeof(int))), (uint64_t) num_keys);
...
extern "C" int test_get_key_after_bytes_unit(void);
int test_get_key_after_bytes_unit(void) {
    int r;
