...
insert_row(DB_ENV *env UU(), DB_TXN *txn, DB *db, uint64_t rowi) {
    ...
    // generate the val
    char *val_buffer = (char *)toku_xmalloc(1024 * sizeof(char));
    memset(val_buffer, 0, sizeof(char) * 1024);

    DBT key = { .data = key_buffer, .size = sizeof key_buffer };
    DBT value = { .data = val_buffer, .size = sizeof(char)*1024 };
...
run_test(DB_ENV *env, int ndbs, int do_txn, uint32_t pagesize, uint64_t nrows) {
    ...
    //DB *dbs[ndbs];
    DB ** dbs = (DB **) toku_xmalloc(sizeof(DB*) * ndbs);
