...
static void cleanup (void) {
    { int chk_r = envt->close(envt, 0); CKERR(chk_r); }
}
...
static void run_test(bool prelock, bool commit) {
    ...
    if (commit) {
        ...
        IN_TXN_COMMIT(envt, NULL, txn_3, 0, {
        ...
        IN_TXN_ABORT(envt, NULL, txn_2, 0, {
                { int chk_r = do_updates(txn_2, db, update_flags); CKERR(chk_r); }
        });
        DBC *cursor = NULL;
        DBT key, val;
...
extern "C" int test_test_update_with_empty_table(void);
int test_test_update_with_empty_table(void) {
    pre_setup();
    run_test(true,true);
    run_test(false,true);
    run_test(true,false);
