...
setup_dbs (void) {
    ...
    uint32_t env_open_flags = DB_INIT_LOG | DB_CREATE | DB_PRIVATE | DB_INIT_MPOOL;
    ...
    init_txn('\0', 0);
    r = db->open(db, txns[(int)'\0'], TOKU_TEST_DATA_DB_NAME, NULL, DB_BTREE, DB_CREATE | DB_READ_UNCOMMITTED, 0600);
        CKERR(r);
    commit_txn('\0');
...
extern "C" int test_test_db_txn_locks_read_uncommitted(void);
int test_test_db_txn_locks_read_uncommitted(void) {
    pre_setup();
    test();
    post_teardown();
    return 0;
