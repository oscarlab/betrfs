...
test_txn_close_before_prepare_commit (void) {
    ...
    r = db->open(db, NULL, TOKU_TEST_DATA_DB_NAME, 0, DB_BTREE, DB_CREATE|DB_AUTO_COMMIT, S_IRWXU+S_IRWXG+S_IRWXO); assert(r == 0);
...
extern "C" int test_test_txn_close_before_prepare_commit(void);
int test_test_txn_close_before_prepare_commit(void) {
    pre_setup();
