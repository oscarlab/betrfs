...
static volatile bool run_test; // should be volatile since we are communicating through this variable.
    ...
    for (int op = 0; op < (int) NUM_OPERATION_TYPES; ++op) {
        ...
        for (int t = 0; t < num_threads; ++t) {
            ...
            if (cli_args->print_thread_performance) {
                const uint64_t persecond = (this_iter * 1000) / secondsthisiter;
                printf("\t%" PRIu64 " (%" PRIu64 " 1000/s)",
                       this_iter, persecond);
#if 0
        ...
        const uint64_t totalpersecond = (period_total * 1000) / secondsthisiter;
        printf("\t%" PRIu64 " (%" PRIu64 " 1000/s)\n",
...
human_print_perf_totals(const struct cli_args *cli_args, uint64_t *counters[], const int num_threads)
{
    ...
    for (int t = 0; t < num_threads; ++t) {
        ...
        for (int op = 0; op < (int) NUM_OPERATION_TYPES; ++op) {
            const uint64_t current = counters[t][op];
            if (cli_args->print_thread_performance) {
                const uint64_t persecond = (current * 1000) /  cli_args->num_seconds;
                printf("\t%s\t%" PRIu64 " (%" PRIu64 " 1000/s)",
                        operation_names[op], current, persecond);
#if 0
    ...
    printf("All threads: ");
    for (int op = 0; op < (int) NUM_OPERATION_TYPES; ++op) {
        const uint64_t totalpersecond = (overall_totals[op] * 1000) / cli_args->num_seconds;
        printf("\t%s\t%" PRIu64 " (%" PRIu64 " 1000/s)", operation_names[op], overall_totals[op], totalpersecond);
#if 0
...
static inline void parse_stress_test_args (struct cli_args *args) {
...
/*wkj: here we start parsing the "extra" arguments, so decide on some sensible defaults to take? */
            bool found = false;
