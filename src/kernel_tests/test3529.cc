...
//    char hello[30], there[30];
    ...
    char *hello = (char*)toku_xmalloc(30);
    char *there = (char*)toku_xmalloc(30);

    snprintf(hello, 30, "hello%04d", i);
    snprintf(there, 30, "there%d", i);
    DBT key, val;
    if (db) {
    int r=db->put(db, txn,
...
extern "C" int test_test3529(void);
int test_test3529(void) {
    did_nothing = 0;
    pre_setup();
    run_test();
    post_teardown();
