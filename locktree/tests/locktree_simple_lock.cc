...
#define ACQUIRE_LOCK(txn, left, right, conflicts) \
        ...
        printk(KERN_ALERT "ACQUIRE_LOCK(txnid_a, one, one)");
        r = ACQUIRE_LOCK(txnid_a, one, one, nullptr);
        invariant(r == 0);
        printk(KERN_ALERT "ACQUIRE_LOCK(txnid_b, two, two)");
        r = ACQUIRE_LOCK(txnid_b, two, two, nullptr);
        invariant(r == 0);
        printk(KERN_ALERT "ACQUIRE_LOCK(txnid_c, three, three)");
        r = ACQUIRE_LOCK(txnid_c, three, three, nullptr);
        invariant(r == 0);
        printk(KERN_ALERT "ACQUIRE_LOCK(txnid_d, four, four)");
        r = ACQUIRE_LOCK(txnid_d, four, four, nullptr);
        invariant(r == 0);
        printk(KERN_ALERT "release_lock(txnid_a, one, one)");
        locktree_test_release_lock(lt, txnid_a, one, one);
        printk(KERN_ALERT "release_lock(txnid_b, two, two)");
        locktree_test_release_lock(lt, txnid_b, two, two);
        printk(KERN_ALERT "release_lock(txnid_c, three, three)");
        locktree_test_release_lock(lt, txnid_c, three, three);
        printk(KERN_ALERT "release_lock(txnid_d, four, four)");
        locktree_test_release_lock(lt, txnid_d, four, four);
        printk(KERN_ALERT "done 4 txns, 4 points, test_run: %d\n", test_run);
        invariant(no_row_locks(lt));
        printk(KERN_ALERT "invariant held\n");
        ...
        invariant(no_row_locks(lt));
        printk(KERN_ALERT "done 2 txns, 2 ranges, test_run: %d\n", test_run);
        ...
        invariant(no_row_locks(lt));
        printk(KERN_ALERT "done 2 txns, 1 range, 1 point, test_run: %d\n",
               test_run);
#undef ACQUIRE_LOCK
    }
    ...
    printk(KERN_ALERT "permuting keys:");
    for (int64_t i = 0; i < num_locks; i++) {
    ...
    for (int64_t i = 0; i < num_locks; i++) {
        printk(KERN_ALERT "\tkey[%d] = %lu\n", i, keys[i]);
    }

    r = mgr.set_max_lock_memory((num_locks + 1) * 500);
    invariant_zero(r);
    printk(KERN_ALERT "set max lock memory", (num_locks + 1) * 500);
    DBT k;
    k.ulen = 0;
    ...
    printk(KERN_ALERT "acquired read locks");

    for (int64_t i = 0; i < num_locks; i++) {
        k.data = (void *) &keys[i];
        printk(KERN_ALERT "releasing lock %lu", keys[i]);
        locktree_test_release_lock(lt, txnid_a, &k, &k);
    }
    printk(KERN_ALERT "released read locks");
    toku_free(keys);
