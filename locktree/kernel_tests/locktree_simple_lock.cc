...
void locktree_unit_test::test_simple_lock(void) {
    ...
    locktree *lt = p_mgr->get_lt(dict_id, desc, compare_dbts, nullptr);
...
#undef ACQUIRE_LOCK
    ...
    r = p_mgr->set_max_lock_memory((num_locks + 1) * 500);
    invariant_zero(r);
...
extern "C" int test_locktree_simple_lock(void);
int test_locktree_simple_lock(void) {
