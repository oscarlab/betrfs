...
static uint64_t get_escalation_count(locktree::manager * p_mgr) {
    uint64_t ret = 0;
    LTM_STATUS_S * p_ltm_status = (LTM_STATUS_S *)toku_xmalloc(sizeof(LTM_STATUS_S));
    p_mgr->get_status(p_ltm_status);

    TOKU_ENGINE_STATUS_ROW key_status = NULL;
    // lookup keyname in status
    for (int i = 0; ; i++) {
        TOKU_ENGINE_STATUS_ROW status = &(p_ltm_status->status[i]);
    ...
    assert(key_status);
    ret = key_status->value.num;
    toku_free(p_ltm_status);
    return ret;
}

extern "C" int test_locktree_escalation_stalls(void);
int test_locktree_escalation_stalls(void) {
    uint64_t stalls = 0;
    int r;
    killed = 0;
    // create a manager
    locktree::manager * p_mgr = (locktree::manager *) toku_xmalloc(sizeof(locktree::manager));
    p_mgr->create(nullptr, nullptr, e_callback, nullptr);
    p_mgr->set_max_lock_memory(1000000000);

    // create lock trees
    DESCRIPTOR desc_0 = nullptr;
    DICTIONARY_ID dict_id_0 = { 1 };
    locktree *lt_0 = p_mgr->get_lt(dict_id_0, desc_0, compare_dbts, nullptr);

    DESCRIPTOR desc_1 = nullptr;
    DICTIONARY_ID dict_id_1 = { 2 };
    locktree *lt_1 = p_mgr->get_lt(dict_id_1, desc_1, compare_dbts, nullptr);

    // create the worker threads
    struct arg big_arg = { p_mgr, lt_0, 1000 };
    pthread_t big_id;
    ...
    p_mgr->release_lt(lt_0);
    p_mgr->release_lt(lt_1);
    p_mgr->destroy();
