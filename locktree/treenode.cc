...
/* wkj: deep recursion broke the kernel on big-nested-{commit|abort}+
template <class F>
...
template <class F>
void treenode::traverse_overlaps(const keyrange &range, F *function) {
    GenericStack<struct traverse_iter *> stack(100);
    treenode *left = NULL;
    treenode *right = NULL;
    struct traverse_iter *XMALLOC(curr);
    curr->tnode = this;
    curr->done_left = false;
    curr->done_overlap = false;
    curr->done_right = false;
    curr->should_unlock = false;

    stack.push(curr);

    while (!stack.is_empty()) {
        curr = stack.top();
        keyrange::comparison c = range.compare(curr->tnode->m_cmp,
                                               curr->tnode->m_range);
        if (c == keyrange::comparison::EQUALS) {
            // Doesn't matter if fn wants to keep going, there
            // is nothing left, so return.
            function->fn(curr->tnode->m_range, curr->tnode->m_txnid);
            if (curr->should_unlock)
                curr->tnode->mutex_unlock();
            stack.pop();
            toku_free(curr);
            continue;
        }
        ...
        if (!curr->done_left) {
            curr->done_left = true;
            left = curr->tnode->m_left_child.get_locked();
            if (left) {
                if (c != keyrange::comparison::GREATER_THAN) {
                    // Target range is less than this node, or it overlaps this
                    // node.  There may be something on the left.
                    struct traverse_iter *XMALLOC(it);
                    it->tnode = left;
                    it->done_left = false;
                    it->done_overlap = false;
                    it->done_right = false;
                    it->should_unlock = true;
                    stack.push(it);
                    continue;
                }
                left->mutex_unlock();
        ...
        if (!curr->done_overlap) {
            curr->done_overlap = true;
            if (c == keyrange::comparison::OVERLAPS) {
                bool keep_going = function->fn(curr->tnode->m_range,
                                               curr->tnode->m_txnid);
                if (!keep_going) {
                    if (curr->should_unlock)
                        curr->tnode->mutex_unlock();
        ...
        if (!curr->done_right) {
            curr->done_right = true;
            right = curr->tnode->m_right_child.get_locked();
            if (right) {
                if (c != keyrange::comparison::LESS_THAN) {
                    // Target range is > this node, or it overlaps this
                    // node.  There may be something on the right.
                    struct traverse_iter *XMALLOC(it);
                    it->tnode = right;
                    it->done_left = false;
