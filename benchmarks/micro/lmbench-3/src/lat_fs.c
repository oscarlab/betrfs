...
measure(size_t size, int parallel, int warmup, int repetitions, void* cookie)
{
	...
	if (gettime()) {
		fprintf(stderr, "\tN=%lu\tmedian=%.0f [mean=%.0lf+/-%.0lf]", 
			(unsigned long)get_n(), 
			(double)(1000000. * get_n() / (double)gettime()),
			getmeanratetime(), 
			ci_width(sqrt(getvarianceratetime()), TRIES));
		/* multiply by 1000000?: dividing by us, but want s */
		/* final units are ops/sec (higher is better) */
	} else {
		fprintf(stderr, "\t-1\t-1");
	}
	...
	benchmp(setup_rm, benchmark_rm, cleanup_names, 0, parallel,
		warmup, repetitions, cookie);
	if (gettime()) {
		fprintf(stderr, "\tmedian=%.0f [mean=%.0lf+/-%.0lf]", 
			(double)(1000000. * get_n() / (double)gettime()),
			getmeanratetime(), 
			ci_width(sqrt(getvarianceratetime()), TRIES));
