...
bench_catch(int parallel, int warmup, int repetitions)
{
	...
	send_usecs = gettime();
	send_n = get_n();
	send_var = getvariancetime();
	send_mean = getmeantime();
	...
	if (gettime() > (send_usecs * get_n()) / send_n) {
		...
		setmeantime(getmeantime() - send_mean);
		setvariancetime(getvariancetime() + send_var);
	} else {
		settime(0);
		setmeantime(0);
		setvariancetime(0);
...
bench_prot(char* fname, int parallel, int warmup, int repetitions)
{
	...
	catch_usecs = gettime();
	catch_n = get_n();
	catch_mean = getmeantime();
	catch_var = getvariancetime();
	...
	if (gettime() > (catch_usecs * get_n()) / catch_n) {
		...
		setmeantime(getmeantime() - catch_mean);
		setvariancetime(getvariancetime() + catch_var);
	} else {
		settime(0);
		setmeantime(0);
		setvariancetime(0);
