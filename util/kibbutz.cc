...
static void *work_on_kibbutz (void *kidv) {
    ...
    while (1) {
        ...
        if (k->please_shutdown) {
            ...
            kunlock(k);
	    /*
		commenting it for now. Since I can't figure out a way to put an implementation 
		of pthread_exit in ftfs module.
	    */
            //toku_pthread_exit(NULL);
            return NULL;
        }
...
//  Note: It is an error for to perform kibbutz_enq operations after this is called.
{
    ...
    for (int i=0; i<k->n_workers; i++) {
        ...
        assert(r==0);
	/*
		commenting it for now. Should be uncommented after pthread_exit() is implemented.
	*/
        //assert(result==NULL);
    }
    toku_free(k->workers);
