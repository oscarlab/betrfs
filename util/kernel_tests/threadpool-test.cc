...
my_thread_f (void *arg) {
    ...
    toku_mutex_unlock(&my_threadpool->mutex);
    printf("%lu:%s:exit\n", (unsigned long)pthread_self(), __FUNCTION__); 
    return arg;
}
...
static int test_threadpool_nofailmalloc(int);
int test_threadpool_nofailmalloc(int max_threads){
    ...
    printf("test threadpool_set_busy\n");
    ...
    #ifndef NO_FAILED_MALLOC 
    //this test is not applicable to toku_memory.c
    if (do_malloc_fail) {
        printf("test threadpool_create with malloc failure\n");
...
extern "C" int test_threadpool(void);
int test_threadpool () {
    test_threadpool_nofailmalloc(1);
    test_threadpool_nofailmalloc(5);
    test_threadpool_nofailmalloc(10);
    test_threadpool_nofailmalloc(20);
    test_threadpool_nofailmalloc(100);
   
     #ifdef NO_FAILED_MALLOC
      printf("\n Failed malloc test is skipped because toku_memory does not support customized alloc yet");
    #endif
    return 0;
}
