...
static void run_test(uint32_t nelts) {
    ...
    bool *called = (bool*) toku_xmalloc(nelts);
    memset(called, 0, nelts);
...
static void stress_deleter(struct reader_extra *const readers, int num_marker_threads, stress_omt *omt) {
    ...
    memset(is_marked_according_to_iterate, 0,
           omt->size() * sizeof(*is_marked_according_to_iterate));
...
static void stress_test(int nelts) {
    ...
    memset(readers, 0, num_marker_threads * sizeof(reader_extra));

    srandom(time(NULL));
    toku_pthread_t *marker_threads =
...
extern "C" int test_marked_omt(void);

int test_marked_omt(void) {
