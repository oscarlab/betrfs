...
static long long best_newbrt_time= 1000000000000;
...
static long long best_frwlock_time= 1000000000000;;
static long long best_frwlock_prelocked_time= 1000000000000;;
...
int fcall_nop_2 (int i) __attribute__((__noinline__));
int fcall_nop_2 (int i) {
    return i;
}
...
int test_rwlock () {
    if (timing_only) {
        if (1) { // to make it easy to only time the templated frwlock
            ...
            time_newbrt_rwlock(); // OK
            time_newbrt_prelocked_rwlock(); // OK
            //time_toku_cv_fair_rwlock();
            //time_toku_fair_rwlock();
        }
	time_frwlock(); // OK
	time_frwlock_prelocked(); // OK
	if (verbose>0) {
            if (1) { // to make it easy to only time the templated frwlock
              ...
              //  printf("//  Best mutex            time=%lldns\n", best_mutex_time);
                ...
                printf("// WARNING: rwlock is not compared fairly please adjust the loops after performance issue of nested lock is fixed\n");
                printf("//  Best newbrt rwlock    time=%lldns\n", best_newbrt_time);
            ...
            printf("//  Best frwlock         time=%lldns\n", best_frwlock_time);
            printf("//  Best frwlock_pre     time=%lldns\n", best_frwlock_prelocked_time);
	}
    } else {
	//test_rwlock();
    }
