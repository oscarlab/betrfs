...
static volatile bool producers_joined = false;
...
static void test_with_threads(void) {
    ...
    uint32_t *array = (uint32_t *) toku_xmalloc(asize * sizeof(uint32_t)); 

    toku::circular_buffer<uint32_t> buf;
    ZERO_STRUCT(buf);
    buf.init(array, asize);
    printf( KERN_INFO "Init the buffer done \n");
    ...
    invariant_zero(r);
    printf( KERN_INFO "Created consumer \n");
    ...
    for (int i = 0; i < nproducers; ++i) {
        ...
        invariant_zero(r);
        printf( KERN_INFO "Created producer \n");
    }
    ...
    invariant(swapped);
    printf( KERN_INFO "Producer joined \n");
    ...
    // delete verbose by yjiao
 
    running = false;
    producers_joined = false;
    toku_free(array);   

    printf( KERN_INFO "%" PRIu32 "\n", extra.xorsum);


}

extern "C" int test_circular_buffer(void);

int test_circular_buffer(void) {
