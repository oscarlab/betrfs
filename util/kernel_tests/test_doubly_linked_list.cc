...
static void test_doubly_linked_list_randomly(void) {
    ...
    if (FTFS_VERBOSE) printf("\n");

    for (int i=0; i<N*N; i++) {
	int x = random()%N;
	if (in[x]) {
	    if (random()%2==0) {
		if (FTFS_VERBOSE) printf("%dR%d ", i, x);
		l.remove(&elts[x]);
		in[x]=false;
	    } else {
		LinkedListElement<int> *re;
		...
		assert(in[v]);
		in[v]=false;
		if (FTFS_VERBOSE) printf("%dP%d ", i, v);
	    }
	} else {
	    l.insert(&elts[x], x);
	    in[x]=true;
	    if (FTFS_VERBOSE) printf("%dI%d ", i, x);
	}
	...
	assert(in[v]);
	in[v]=false;
	if (FTFS_VERBOSE) printf("P%d ", v);
    }
    for (int i=0; i<N; i++) assert(!in[i]);
    if (FTFS_VERBOSE) printf("\n");
}

extern "C" int test_doubly_list(void); 
