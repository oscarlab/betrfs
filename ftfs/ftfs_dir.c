...
#endif
typedef int (* sys_chdir_t) (char * path);
DECLARE_SYMBOL_FTFS(sys_chdir);
...
retry:
	...
	/* we are in the southbound context, so ignore permissions */
...
char *getcwd(char *buf, int buflen)
{
	...
	pwd = ftfs_fs->pwd; /* wkj: potentially racey? we do not hold
			     * ftfs_southbound_lock, but ftfs_fs only
...
int chdir(char * path) {
/* used in ftfs unit tests only. do not optimize */
    return return_errno_nonzero(ftfs_sys_chdir(path));
}
