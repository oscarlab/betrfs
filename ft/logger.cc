...
int toku_logger_create (TOKULOGGER *resultp) {
    ...
    if (result==0) return get_error_errno(-ENOMEM);
...
static int open_logdir(TOKULOGGER logger, const char *directory) {
    ...
    if (logger->directory==0) return get_error_errno(-ENOMEM);
...
// Exit:  This thread has permission to modify the output.
{
    ...
    r = close(logger->fd);                               if (r!=0) return get_error_errno(r);
...
// This is called during logger initialalization, and no locks are required.
{
    ...
    struct dirent *de;
    if (d==0) return get_error_errno(-ENOSYS);
    while ((de=readdir(d))) {
        if (de==0) return get_error_errno(-ENOSYS);
        long long thisl;
...
// Entry and Exit: This thread has permission to modify the output.
{
    ...
    if (logger->write_log_files) {
        ...
        if (logger->fd < 0) {
            return get_error_errno(logger->fd);
        }
        fsync_logdir(logger);
