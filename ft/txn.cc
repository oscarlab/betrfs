...
// Recursion has failed test xid_lsn_independent in the kernel.
// Use the iteartion verison instead -JYM
...
typedef struct tokutxn_args {
    TOKUTXN txn;
} tokutxn_args;

static inline tokutxn_args * pack_args(TOKUTXN txn) {
    tokutxn_args * XMALLOC(cur_args);
    cur_args -> txn =  txn;
    return cur_args;
}
...
maybe_log_begin_txn_for_write_operation_unlocked(TOKUTXN txn) {
    ...
    stack.push(e1);
    tokutxn_args * cur_args = pack_args(txn);
    ...
    while(!stack.is_empty()){
        switch (status) {
            case ENTRY: {
                cur_args = (tokutxn_args *) stack.pop().addr;
                TOKUTXN cur_txn = cur_args -> txn;
                if(cur_txn->begin_was_logged) {
                    //base case #1
                    status = stack.pop().status;
                    toku_free(cur_args);
                } else {
                    TOKUTXN parent;
                    parent = cur_txn->parent;
                    TXNID_PAIR xid;
                    xid = cur_txn->txnid;
                    TXNID_PAIR pxid;
                    pxid = TXNID_PAIR_NONE;
                    if (parent) { // recursion
                        ...
                        stack.push(e4);
                        tokutxn_args * parent_args = pack_args(parent);
                        stack_elem e5 = {.addr = (void*) parent_args};
                        stack.push(e5);
                        status = marker::ENTRY;
                    } else { //base case #2
                        status = stack.pop().status;
                        toku_log_xbegin(cur_txn->logger, NULL,0, xid,pxid);
                        cur_txn->begin_was_logged = true;
                        toku_free(cur_args);
                    }
                }
                break;
            ...
            case BACKTRACK: {
                    ...
                    status = stack.pop().status;
                    TOKUTXN cur_txn = cur_args -> txn;
                    TXNID_PAIR xid = cur_txn -> txnid;
                    TOKUTXN parent = cur_txn -> parent;
                    TXNID_PAIR pxid = parent->txnid;
                    toku_log_xbegin(cur_txn->logger, NULL, 0, xid, pxid);
                    cur_txn->begin_was_logged = true;
                    toku_free(cur_args);
                    break;
