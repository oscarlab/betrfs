                          ...
                          /* out */ FIDX *idx)
{
    ...
    if (fi->n_files >= fi->n_files_limit) {
        XREALLOC_N(fi->n_files_limit, fi->n_files_limit * 2, fi->file_infos);
        fi->n_files_limit *=2;
    }
...
/* Effect: add a row to a collection. */
{
    ...
    if (rows->n_rows >= rows->n_rows_limit) {
        ...
        REALLOC_N(rows->n_rows_limit, rows->n_rows_limit * 2, rows->rows);
        rows->n_rows_limit *= 2;
        if (rows->rows == NULL) {
            result = ENOMEM;
            rows->rows = old_rows;
    ...
    if (next_off > rows->n_bytes_limit) {
        ...
        REALLOC_N(rows->n_bytes_limit, old_n_bytes_limit, rows->data);
        if (rows->data == NULL) {
            result = ENOMEM;
            rows->data = old_data;
 ...
 */
{
    ...
    if (fs->n_temp_files+1 > fs->n_temp_files_limit) {
        int old_temp_files_limit = fs->n_temp_files_limit;
        fs->n_temp_files_limit = (fs->n_temp_files+1)*2;
        XREALLOC_N(old_temp_files_limit, fs->n_temp_files_limit, fs->data_fidxs);
    }
...
static void allocate_node (struct subtrees_info *sts, int64_t b) {
    if (sts->n_subtrees >= sts->n_subtrees_limit) {
        XREALLOC_N(sts->n_subtrees_limit, sts->n_subtrees_limit * 2, sts->subtrees);
...
// dbuf will always contained 4096-byte aligned buffer, but the length might not be a multiple of 4096 bytes.  If that's what you want, then pad it.
struct dbuf {
    unsigned char *buf;
    int buflen;
    int off;
...
// Return the new block number
{
    ...
    if (block_number >= out->n_translations_limit) {
        ...
        REALLOC_N(out->n_translations_limit, old_n_translations_limit, out->translation);
...
static int write_literal(struct dbout *out, void*data,  size_t len) {
    invariant(out->current_off%BLOCK_ALIGNMENT==0);
...
static int write_translation_table (struct dbout *out, long long *off_of_translation_p) {
    ...
    // pad it to 4096 zeros
    long long encoded_length = ttable.off;
    {
        int nbytes_to_add = roundup_to_multiple(BLOCK_ALIGNMENT, ttable.off) - encoded_length;
...
write_header (struct dbout *out, long long translation_location_on_disk, long long translation_size_on_disk) {
    ...
    size_t alloced_size = roundup_to_multiple(BLOCK_ALIGNMENT, size);
    struct wbuf wbuf;
