...
static toku_off_t lc_file_start(int fp) {
    struct log_super_block *log_sb = NULL;
    ssize_t r;
    toku_off_t ret;
...
static int lc_open_logfile(TOKULOGCURSOR lc, int index) {
    ...
    lc->cur_fp = open(lc->logfiles[index], O_RDONLY);
    ...
    if ( lc->cur_log_end >= sizeof(struct log_super_block) ) {
        r = lseek(lc->cur_fp, sizeof(struct log_super_block), SEEK_SET);
        if (r!=sizeof(struct log_super_block)) return r;
        r = 0;
    }
    // mark as open
    lc->is_open = true;
...
//        dprintf(STDERR, "Bad LSN: %d %s direction = %d, lsn.lsn = %" PRIu64 ", cur_lsn.lsn=%" PRIu64 "\n",
...
static int lc_log_read(TOKULOGCURSOR lc)
{
    ...
    if (r!=0) {
        ...
        if (r==DB_BADFORMAT) {
            dprintf(STDERR, "%.24s Tokudb bad log format in %s\n", ctime(&tnow), lc->logfiles[lc->cur_logfiles_index]);
        }
        else {
            dprintf(STDERR, "%.24s Tokudb unexpected log format error '%s' in %s\n", ctime(&tnow), strerror(r), lc->logfiles[lc->cur_logfiles_index]);
        }
    }
    return r;
}
...
static int lc_log_read_backward(TOKULOGCURSOR lc)
{
	...
	assert(lc->cur_log_end >= sizeof(struct log_super_block));
        r = lseek(lc->cur_fp, lc->cur_log_end, SEEK_SET);
        assert(0==r);
    ...
    if (r!=0) {
        ...
        if (r==DB_BADFORMAT) {
            ...
            dprintf(STDERR, "%.24s Tokudb uUnexpected log format error '%s' in %s\n", ctime(&tnow), strerror(r), lc->logfiles[lc->cur_logfiles_index]);
        }
    }
    return r;
}
...
int toku_logcursor_next(TOKULOGCURSOR lc, struct log_entry **le) {
    ...
    if (lc->cur_fp >= 0) {
        toku_off_t cur_pos = lseek(lc->cur_fp, 0, SEEK_CUR);
        if (cur_pos == (toku_off_t)lc->cur_log_end) return DB_NOTFOUND;
    }
...
int toku_logcursor_first(TOKULOGCURSOR lc, struct log_entry **le) {
    ...
    // move to "first" entry
    r = lseek(lc->cur_fp, lc->cur_log_start, SEEK_SET);
    if (r != (int) lc->cur_log_start) return r;
    // read the entry
    r = lc_log_read(lc);
    if (r!=0) return r;
...
int toku_logcursor_last(TOKULOGCURSOR lc, struct log_entry **le) {
    ...
    while (1) {
        ...
        if (lc->cur_log_end < sizeof(struct log_super_block))
            return DB_BADFORMAT;
        r = lseek(lc->cur_fp, lc->cur_log_end, SEEK_SET);    assert(r==(int)lc->cur_log_end);
        // read backward
        r = toku_log_fread_backward(lc->cur_fp, &(lc->entry));
        ...
        if (r>0) {
            ...
            time_t tnow = time(NULL);
            dprintf(STDERR, "%.24s Tokudb recovery repairing log\n", ctime(&tnow));
            r = lc_fix_bad_logfile(lc);
            if ( r != 0 ) {
                dprintf(STDERR, "%.24s Tokudb recovery repair unsuccessful\n", ctime(&tnow));
                return DB_BADFORMAT;
            }
            // try reading again
            r = toku_log_fread_backward(lc->cur_fp, &(lc->entry));
...
static int lc_fix_bad_logfile(TOKULOGCURSOR lc) {
    ...
    toku_off_t temp_pos;
    last_good_pos = lseek(lc->cur_fp, 0, SEEK_CUR);
    while (1) {
        // initialize le
        ...
        if ( r!=0 )
            break;
        temp_pos = lseek(lc->cur_fp, 0, SEEK_CUR);
         ...
         */
            if (temp_pos > (toku_off_t)lc->cur_log_end || temp_pos < (toku_off_t)lc->cur_log_start)
                break;
    ...
    assert(fd >= 0);
    if (last_good_pos == (toku_off_t)lc->cur_log_start)
        last_good_pos = 0;
