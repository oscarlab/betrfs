...
toku_serialize_descriptor_contents_to_fd(int fd, const DESCRIPTOR desc, DISKOFF offset) {
    ...
    int64_t size_aligned = roundup_to_multiple(BLOCK_ALIGNMENT, size);
    struct wbuf w;
    char *XMALLOC_N_ALIGNED(BLOCK_ALIGNMENT, size_aligned, aligned_buf);
...
deserialize_descriptor_from(int fd, BLOCK_TABLE bt, DESCRIPTOR desc, int layout_version) {
    ...
    if (size > 0) {
        ...
        {
            ssize_t size_to_malloc = roundup_to_multiple(BLOCK_ALIGNMENT, size);
            XMALLOC_N_ALIGNED(BLOCK_ALIGNMENT, size_to_malloc, dbuf);
            {
...
//   We deserialize brt header only once and then share everything with all the brts.
{
    ...
    {
        size_t size_to_read = roundup_to_multiple(BLOCK_ALIGNMENT, translation_size_on_disk);
        unsigned char *XMALLOC_N_ALIGNED(BLOCK_ALIGNMENT, size_to_read, tbuf);
        {
...
//  file AND the header is useless
{
    ...
    const int64_t prefix_size = 8 + // magic ("tokudata")
                                ...
                                4 + // build_id
                                4;  // size
    const int64_t read_size = roundup_to_multiple(BLOCK_ALIGNMENT, prefix_size);
    unsigned char *XMALLOC_N_ALIGNED(BLOCK_ALIGNMENT, read_size, prefix);
    rb->buf = NULL;
    ...
    *version_p = version;
    ...
    {
        toku_free(rb->buf);
        uint32_t size_to_read = roundup_to_multiple(BLOCK_ALIGNMENT, size);
        XMALLOC_N_ALIGNED(BLOCK_ALIGNMENT, size_to_read, rb->buf);

        assert(offset_of_header%BLOCK_ALIGNMENT==0);
...
void toku_serialize_ft_to (int fd, FT_HEADER h, BLOCK_TABLE blocktable, CACHEFILE cf) {
    ...
    assert(w_translation.size % BLOCK_ALIGNMENT == 0);           // the number of bytes available in the buffer is 0 mod 4096, and those last bytes are all initialized.

    struct wbuf w_main;
    size_t size_main       = toku_serialize_ft_size(h);
    size_t size_main_aligned = roundup_to_multiple(BLOCK_ALIGNMENT, size_main);
    assert(size_main_aligned<BLOCK_ALLOCATOR_HEADER_RESERVE);
    char *XMALLOC_N_ALIGNED(BLOCK_ALIGNMENT, size_main_aligned, mainbuf);
