...
static void do_assert(int die, const char *expr_as_string, const char *function, const char * file, int line, int caller_errno)
{
    if (die) {
        char msg[900];
        snprintf(msg, sizeof(msg), "%s:%d %s: Assertion `%s' failed (errno=%d)\n", file, line, function, expr_as_string, caller_errno);
        perror(msg);
    }

}

#define assert(expr) do_assert((expr) == 0, #expr, __FUNCTION__, __FILE__, __LINE__, errno)
