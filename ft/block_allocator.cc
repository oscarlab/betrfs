...
create_block_allocator (BLOCK_ALLOCATOR *ba, uint64_t reserve_at_beginning, uint64_t alignment) {
    assert(alignment>=BLOCK_ALIGNMENT && 0==(alignment%BLOCK_ALIGNMENT)); // the alignment must be at least 4096 and aligned with 4096 to make DIRECT_IO happy.
...
// Effect: Allocate a block. The resulting block must be aligned on the ba->alignment (which to make direct_io happy must be a positive multiple of 4096).
{
