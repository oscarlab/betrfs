...
verify_snapshot_system(TXN_MANAGER txn_manager UU()) {
    ...
    TOKUTXN * snapshot_txns = (TOKUTXN *) toku_xmalloc(sizeof(TOKUTXN)* num_snapshot_txnids);
    TOKUTXN * live_txns = (TOKUTXN *) toku_xmalloc(sizeof(TOKUTXN) * num_live_txns);
    struct referenced_xid_tuple ** referenced_xid_tuples = (struct referenced_xid_tuple **) toku_xmalloc(sizeof(struct referenced_xid_tuple *) * num_referenced_xid_tuples);    
#endif
...
// do this to get an omt of snapshot_txnids
    ...
    toku_free(snapshot_txnids);
    toku_free(snapshot_txns);
    toku_free(live_txns);
    toku_free(referenced_xid_tuples);
#endif
}
