			...
			)
{
	...
	int r = toku_test_cachetable_unpin(cf_f1, make_blocknum(100), 100, CACHETABLE_CLEAN, attr);
...
run_test (void) {
	...
	r = toku_cachetable_openf(&cf_f1, ct, fname1, O_RDWR|O_CREAT, S_IRWXU|S_IRWXG|S_IRWXO); assert(r == 0);
	...
	r = toku_cachetable_get_and_pin(cf_f1, make_blocknum(100), 100, &vs[4], &ss[4],
			wc,
			def_fetch,
			def_pf_req_callback,
			def_pf_callback,
	...
	PAIR_ATTR attr = make_pair_attr(8);
	attr.cache_pressure_size = 100;
	r = toku_test_cachetable_unpin(cf_f1, make_blocknum(100), 100, CACHETABLE_CLEAN, attr);

	for (int i = 0; i < 4; ++i) {
		r = toku_cachetable_get_and_pin(cf_f1, make_blocknum(i+1), i+1, &vs[i], &ss[i],
				wc,
				def_fetch,
				def_pf_req_callback,
				def_pf_callback,
		...
		r = toku_test_cachetable_unpin(cf_f1, make_blocknum(i+1), i+1, CACHETABLE_CLEAN, attr);
		assert_zero(r);
	}
	...
	toku_cachefile_close(&cf_f1, false, ZERO_LSN);
	toku_cachetable_close(&ct);
}

extern "C" int test_cachetable_cleaner_thread_simple(void);
int test_cachetable_cleaner_thread_simple(void) 
{
	initialize_dummymsn();
