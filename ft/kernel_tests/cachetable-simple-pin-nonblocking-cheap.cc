...
static void flush (CACHEFILE f __attribute__((__unused__)),
		int UU(fd),
...
static void unlock_dummy (void* UU(v)) {
}
...
static void run_case_that_should_succeed(CACHEFILE f1, pair_lock_type first_lock, pair_lock_type second_lock) 
{
	void* v1;
	long s1;
...
static void run_case_that_should_fail(CACHEFILE f1, pair_lock_type first_lock, pair_lock_type second_lock) 
{
	void* v1;
	long s1;
	CACHETABLE_WRITE_CALLBACK wc = def_write_callback(NULL);
...
extern "C" int test_cachetable_simple_pin_nonblocking_cheap(void);
int test_cachetable_simple_pin_nonblocking_cheap(void) {
	initialize_dummymsn();
