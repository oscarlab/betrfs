...
void checkpointer_test::test_begin_checkpoint() {
    cachefile_list * cflp = (cachefile_list *) toku_xmalloc(sizeof(cachefile_list));
    memset(cflp,0, sizeof(cachefile_list));
    cflp->init();

    cachetable * ctblp = (cachetable*) toku_xmalloc(sizeof(cachetable));
    memset(ctblp,0, sizeof(cachetable));
    ctblp->list.init();

    ZERO_STRUCT(m_cp);
    m_cp.init(&(ctblp->list), NULL, &(ctblp->ev), cflp);
    ...
    //cachefile cf;
    struct cachefile * cfp = (struct cachefile *) toku_xmalloc(sizeof(struct cachefile));
    cfp->next = NULL;
    cfp->for_checkpoint = false;
    m_cp.m_cf_list->m_active_head = cfp;
    create_dummy_functions(cfp);

    m_cp.begin_checkpoint();
    assert(m_cp.m_checkpoint_num_files == 1);
    assert(cfp->for_checkpoint == true);

    // 3. Call checkpoint with MANY cachefiles.
    const uint32_t count = 3;
    struct cachefile * cfsp = (struct cachefile *) toku_xmalloc(count*sizeof(struct cachefile));
    m_cp.m_cf_list->m_active_head = cfsp;
    for (uint32_t i = 0; i < count; ++i) {
        cfsp[i].for_checkpoint = false;
        create_dummy_functions(cfsp+i);
        if (i == count - 1) {
            cfsp[i].next = NULL;
        } else {
            cfsp[i].next = &cfsp[i + 1];
        }
    }
    ...
    for (uint32_t i = 0; i < count; ++i) {
        assert(cfsp[i].for_checkpoint == true);
    }
    ctblp->list.destroy();
    m_cp.destroy();
    toku_free(cflp);
...
void checkpointer_test::test_pending_bits() {
    ...
    ctpair * p_pair = (ctpair*) toku_xmalloc(sizeof(ctpair));
    ...
    m_cp.m_list->put(p_pair);

    m_cp.turn_on_pending_bits();
    assert(p_pair->checkpoint_pending);
    m_cp.m_list->evict_completely(p_pair);

    //
    // 3. Many hash chain entries.
    //
    const uint32_t count = 3;
    ctpair * p_pairs= (ctpair *) toku_xmalloc(count*sizeof(ctpair));
    memset(p_pairs, 0 ,sizeof(ctpair)*count); 
    //ZERO_ARRAY(pairs);
    add_pairs(&cf, p_pairs, count, 0);

    m_cp.turn_on_pending_bits();

    for (uint32_t i = 0; i < count; ++i) {
        assert(p_pairs[i].checkpoint_pending);
    }
    for (uint32_t i = 0; i < count; ++i) {
        CACHEKEY key;
        key.b = i;
...
void checkpointer_test::test_end_checkpoint() {
    ...
    ctpair * pairs = (ctpair *) toku_xmalloc(count*sizeof(ctpair));
    memset(pairs, 0, count*sizeof(ctpair));
...
extern "C" int test_cachetable_checkpointer_class(void);
int
test_cachetable_checkpointer_class(void) {
    int r = 0;
    initialize_dummymsn();
