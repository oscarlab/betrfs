...
static void *f2_pin(void *arg) {
    ...
    // cause a partial eviction of f1_5097's PAIR, reducing its size from 8 to 1
...
cachetable_test (void) {
    ...
    for (int i = 0; i < 20; i++) {
        r = toku_cachetable_get_and_pin(f1_5097, make_blocknum(1), 1, &v1, &s1, wc, def_fetch, def_pf_req_callback, def_pf_callback, true, NULL);
        assert(r == 0);
        r = toku_test_cachetable_unpin(f1_5097, make_blocknum(1), 1, CACHETABLE_DIRTY, make_pair_attr(8));
        assert(r == 0);
    }

    // at this point, we have a dirty PAIR in the cachetable associated with cachefile f1_5097
    ...
    usleep(2*1024*1024);
    check_flush = true;
    toku_cachefile_close(&f1_5097, false, ZERO_LSN); 
...
extern "C" int test_cachetable_5097(void); 

int test_cachetable_5097(void){
  initialize_dummymsn();
