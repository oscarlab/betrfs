...
extern "C" int test_logcursor(void);

int test_logcursor(void) {
...
int test_0 (void) {
    ...
    r = toku_logcursor_create(&cursor, LOGDIR);   
	assert(r==0);
    r = toku_logcursor_next(cursor, &entry);     
	 assert(r==0);
    r = toku_logcursor_next(cursor, &entry);      
	 assert(r==0);
    r = toku_logcursor_next(cursor, &entry);      
	...
	assert(r==0);
    r = toku_logcursor_prev(cursor, &entry);  
 	assert(r==0);
    r = toku_logcursor_prev(cursor, &entry); 
 	assert(r==0);
    r = toku_logcursor_destroy(&cursor);   
...
char logfile_cursor[PATH_MAX];

int test_1 () {
    int r=0;
    sprintf(logfile_cursor, "log000000000000.tokulog%d", TOKU_LOG_VERSION);

    struct toku_logcursor *cursor;
    struct log_entry *entry;

    r = toku_logcursor_create_for_file(&cursor, LOGDIR, logfile_cursor);   
    assert(r==0);
