...
doit (void) {
    ...
    r = toku_open_ft_handle(TOKU_TEST_FILENAME_DATA, 1, &t, NODESIZE, NODESIZE/2, TOKU_DEFAULT_COMPRESSION_METHOD, ct, null_txn, toku_builtin_compare_fun);
    assert(r==0);
...
extern "C" int test_hot_with_bounds(void);
int
test_hot_with_bounds (void) {
    initialize_dummymsn();
