...
static FT_HANDLE ft_handle_t;

static bool checkpoint_called;
static bool checkpoint_callback_called;
static toku_pthread_t checkpoint_tid;
...
doit (bool after_split) {
	...
	r = toku_testsetup_leaf(ft_handle_t, &node_leaf, 1, NULL, NULL);
	assert(r==0);

	r = toku_testsetup_nonleaf(ft_handle_t, 1, &node_root, 1, &node_leaf, 0, 0);
	assert(r==0);

	r = toku_testsetup_root(ft_handle_t, node_root);
	assert(r==0);

	char *dummy_val = (char *) toku_malloc(sizeof(char) * (NODESIZE-50));
	memset(dummy_val, 0, sizeof(char) * (NODESIZE-50));
	...
	// do the flush
	toku_ft_flush_some_child(ft_handle_t->ft, node, &fa);
	...
	DBT k;
	struct check_pair pair1 = {2, "a", sizeof(char) * (NODESIZE-50), dummy_val, 0};
	r = toku_ft_lookup(c_ft, toku_fill_dbt(&k, "a", 2), lookup_checkf, &pair1);
	assert(r==0);
	struct check_pair pair2 = {2, "z", sizeof(char) * (NODESIZE-50), dummy_val, 0};
	r = toku_ft_lookup(c_ft, toku_fill_dbt(&k, "z", 2), lookup_checkf, &pair2);
	assert(r==0);

        toku_free(dummy_val);
	r = toku_close_ft_handle_nolsn(ft_handle_t, 0);    assert(r==0);
	r = toku_close_ft_handle_nolsn(c_ft, 0);    assert(r==0);
	toku_cachetable_close(&cacheTable);
        toku_flusher_thread_set_callback(NULL, NULL);
}
