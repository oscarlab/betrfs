...
static void cachetable_checkpoint_test(int n, enum cachetable_dirty dirty) {
    ...
    r = toku_checkpoint(cp, NULL, checkpoint_callback, &callback_was_called, checkpoint_callback2, &callback2_was_called, CLIENT_CHECKPOINT, false);
    assert(r == 0);
...
extern "C" int test_cachetable_checkpoint(void);
int
test_cachetable_checkpoint(void) {
    int i;
    callback_was_called = 0;
    callback2_was_called = 0;
    initialize_dummymsn();
