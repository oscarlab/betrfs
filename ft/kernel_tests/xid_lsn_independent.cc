...
#define ENVDIR TOKU_TEST_ENV_DIR_NAME
...
static void test_xid_lsn_independent(int N) {
    ...
    r = toku_open_ft_handle(TOKU_TEST_FILENAME_DATA, 1, &brt, 1024, 256, TOKU_DEFAULT_COMPRESSION_METHOD, ct, txn, toku_builtin_compare_fun);
    CKERR(r);
    ...
    r = toku_checkpoint(cp, logger, NULL, NULL, NULL, NULL, CLIENT_CHECKPOINT, false);
    CKERR(r);
...
static void test_xid_lsn_independent_shutdown_recovery(int N) {
    TOKULOGGER logger;
    CACHETABLE ct;
    int r=test_setup(TOKU_TEST_ENV_DIR_NAME, &logger, &ct);
    assert(r==0);
...
static void test_xid_lsn_independent_parents(int N) {
    ...
    TOKUTXN * txns_hack = (TOKUTXN*) toku_xmalloc((N+1) * sizeof(TOKUTXN));
    ...
    do {
        ...
        memset(txns_hack, 0, sizeof(TOKUTXN)*(N+1)); 
        for (int i = 0; i < N; i++) {
...
extern "C"  int test_xid_lsn_independent(void);
int test_xid_lsn_independent (void) {
    initialize_dummymsn();
