...
cachetable_test (bool write_first, bool write_second, bool start_checkpoint) {
    ...
    toku_os_recursive_delete(fname1);
    ...
    if (start_checkpoint) {
        assert(v1_written_cc == write_first);
        assert(v2_written_cc == write_second);
    }
...
extern "C" int test_cachetablesimple_put_depnodes(void);

int test_cachetablesimple_put_depnodes(void) {
