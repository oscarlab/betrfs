...
static void simple_test(bool unlink_on_close) 
{
	...
	toku_cachefile_close(&f1, false, ZERO_LSN);
	CACHETABLE_STATUS_S *stats = (CACHETABLE_STATUS_S *)  toku_malloc(sizeof(CACHETABLE_STATUS_S));
	toku_cachetable_get_status(ct, stats);
	assert(f_flush_called);
	assert(close_called);
	assert(write_me);
	if (unlink_on_close) {
		assert(free_called);
...
extern "C" int test_cachetable_simple_close(void);
int test_cachetable_simple_close(void) 
{
	initialize_dummymsn();
