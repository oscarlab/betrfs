...
cachetable_test (bool write_first, bool write_second, bool start_checkpoint) {
    ...
    CACHETABLE_WRITE_CALLBACK wc = def_write_callback(&val1_dd);
    wc.flush_callback = flush;
    wc.write_extraargs = &val1_dd;
    dest_pair_dd = &dependent_pairs[0];
    r = toku_cachetable_get_and_pin(f1, make_blocknum(1), 1, &v1, &s1, wc, fetch, def_pf_req_callback, def_pf_callback, true, &val1_dd);
    dest_pair_dd = &dependent_pairs[1];
    wc.write_extraargs = &val2_dd;
    r = toku_cachetable_get_and_pin(f1, make_blocknum(2), 2, &v2, &s2, wc, fetch, def_pf_req_callback, def_pf_callback, true, &val2_dd);
