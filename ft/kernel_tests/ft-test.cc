...
static void  test_read_what_was_written (void) {
    ...
    {
	DBT* kp,* vp;
        kp = (DBT *) toku_xmalloc(sizeof(DBT));
        vp = (DBT *) toku_xmalloc(sizeof(DBT));
	toku_ft_insert(brt, toku_fill_dbt(kp, "hello", 6), toku_fill_dbt(vp, "there", 6), null_txn);
        toku_free(kp);
        toku_free(vp);
    }
...
//	    char key[100],val[100];
//	    DBT k,v;
            char * key = (char *) toku_xmalloc(sizeof(char)*100);
            char * val = (char *) toku_xmalloc(sizeof(char)*100);
            DBT* kp,* vp;
            kp = (DBT *) toku_xmalloc(sizeof(DBT));
            vp = (DBT *) toku_xmalloc(sizeof(DBT));
	    ...
	    toku_ft_insert(brt, toku_fill_dbt(kp, key, strlen(key)+1), toku_fill_dbt(vp, val, strlen(val)+1), null_txn);
	    if (i<600) {
		int verify_result=toku_verify_ft(brt);
		if (verify_result) {
...
static void ft_blackbox_test (void) {
    ...
    //if (verbose)
    printf("did read_what_was_written\n");
    test_cursor_next();
...
extern "C" int test_ft_test(void);
int
test_ft_test (void) {
    initialize_dummymsn();
