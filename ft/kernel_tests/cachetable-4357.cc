...
{    
	...
	int r = toku_cachetable_get_and_pin_nonblocking(
			caheFile, 
			make_blocknum(1), 
			toku_cachetable_hash(caheFile, make_blocknum(1)), 
			&v1, 
			&s1, 
...
static void cachetable_test (void) 
{
	...
	// sleep 3 seconds
	usleep(3*1024*1024);
	r = toku_test_cachetable_unpin_and_remove(caheFile, make_blocknum(1), NULL, NULL);
	assert_zero(r);
	...
	toku_cachefile_close(&caheFile, false, ZERO_LSN);
	toku_cachetable_close(&ct);
}

extern "C" int test_cachetable_4357(void);
int test_cachetable_4357(void) 
{
	initialize_dummymsn();
