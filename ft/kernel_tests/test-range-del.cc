...
//    assert(r==0);
   ...
   // insert key-value pair in the range [a - e]
    ...
    toku_ft_insert(t,
       ...
       // perform a random lookup to assert on the insertion
    memset(&v, 0, sizeof(v));
    toku_fill_dbt(&k, "b", 2);
    struct check_pair pair = {2, "b", 3, "bb", 0};
    r = toku_ft_lookup(t, &k, lookup_checkf, &pair);
    assert(r==0);
    assert(pair.call_count == 1);

    // insert a multicast delete message
    // delete all key-value pairs between [b - d]
    ...
    assert_zero(r);
         ...
         // perform a random lookup to assert on the insertion
    // now do a lookup on one of the keys in the range [b - d] 
    // it should not find any
    char key[2] = "b";
    ft_lookup_and_fail_nodup(t, key);
    key[0]='c'; 
    ft_lookup_and_fail_nodup(t, key);
    key[0]='d';
    ft_lookup_and_fail_nodup(t, key);
    // and the survived kv
    ft_lookup_and_check_nodup(t,"a","aa");
    ft_lookup_and_check_nodup(t,"e", "ee");
     // close the ft-handle and cachetable
    r = toku_close_ft_handle_nolsn(t, 0);    assert(r==0);
    toku_cachetable_close(&ct);
}

extern "C" int test_range_del(void);
int
test_range_del (void) {
    initialize_dummymsn();
