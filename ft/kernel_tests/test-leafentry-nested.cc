...
extern "C" void generate_committed_for(ULE ule, DBT *val);

//static void
void generate_committed_for(ULE ule, DBT *val) {
    ule->num_cuxrs = 1;
    ule->num_puxrs = 0;
    ule->uxrs = ule->uxrs_static;
    ule->uxrs[0].type   = XR_INSERT;
...
test_le_empty_apply(void) {
    ...
    memcpy(ule_initial, &ule_committed_delete, sizeof(*ule_initial));
        ...
        for (valsize = 0; valsize < MAX_SIZE; valsize += (random() % MAX_SIZE) + 1) {
            for (nesting_level = 0;
                ...
                if (nesting_level > 0) {
                    ...
                    memcpy(ule_expected, &ule_committed_delete,
                           sizeof(*ule_initial));
    ...
    for (valsize = 0; valsize < MAX_SIZE; valsize += (random() % MAX_SIZE) + 1) {
        for (nesting_level = 0;
            ...
            if (nesting_level > 0) {
                ...
                ULE XMALLOC(ule_expected);
                memcpy(ule_expected, ule_initial, sizeof(*ule_initial));
...
extern "C" int test_leafentry_nested (void);
int
