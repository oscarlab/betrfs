...
append_leaf(FTNODE leafnode, void *key, size_t keylen, void *val, size_t vallen) {
    ...
    toku_ft_bn_apply_cmd_once(BLB(leafnode, 0), NULL, &cmd, idx, NULL, TXNID_NONE, make_gc_info(false), NULL, NULL);
...
extern "C" int test_verify_unsorted_pivots (void);
int test_verify_unsorted_pivots (void) 
{
    int height = 1;
    int fanout = 3;
    int nperleaf = 8;
