...
static void *run_expensive_pf(void *arg) {
    ...
    int r = toku_cachetable_get_and_pin_nonblocking(f1_nb, make_blocknum(1), 1, &v1, &s1, wc, sleep_fetch, sleep_pf_req_callback, sleep_pf_callback, PL_READ, NULL, NULL);
...
run_test (void) {
    ...
    r = toku_cachetable_get_and_pin_nonblocking(f1_nb, make_blocknum(1), 1, &v1, &s1, wc, sleep_fetch, def_pf_req_callback, def_pf_callback, PL_READ, NULL, NULL);
    assert_zero(r);
    r = toku_cachetable_get_and_pin_nonblocking(f1_nb, make_blocknum(1), 1, &v1, &s1, wc, sleep_fetch, def_pf_req_callback, def_pf_callback, PL_READ, NULL, NULL);
    assert_zero(r);
    r = toku_test_cachetable_unpin(f1_nb, make_blocknum(1), 1, CACHETABLE_CLEAN, make_pair_attr(8));
    ...
    toku_cachefile_close(&f1_nb, false, ZERO_LSN);
