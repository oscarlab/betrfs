...
run_test(void) {
    ...
    assert(r == 0);
#else
    // YZJ: For SFS we do not ignore empty log, this changes the return value of tokudb_recovery
    // check recover.cc for more details
...
extern "C" int test_recovery_empty(void);

int test_recovery_empty() {
    int r;
