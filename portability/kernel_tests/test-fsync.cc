...
create_files(int N, int fds[/*N*/]) {
    ...
    char name[30];
    for (i = 0; i < N; i++) {
        snprintf(name, sizeof(name), "%s/%d", TOKU_TEST_ENV_DIR_NAME, i);
...
time_many_fsyncs_one_file(int N, int bytes, int fds[/*N*/]) {
    ...
    if (verbose) {
        printf("Fsyncing one file %d times:\n"
               "\tFirst fsync took: [%" PRIu64 "] seconds\n"
               "\tRemaining %d fsyncs took additional: [%" PRIu64 "] seconds\n"
...
extern "C" int test_fsync_files(void);
int test_fsync_options(int, int);
int test_fsync_options(int N, int bytes) {
    int r;
    //N = 1000;
    //bytes = 4096;
