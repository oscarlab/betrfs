...
static int timing_only=1;


static const int T=6;
static const int N=10000000;
static const long long factor = 1000000000;
static long long best_nop_time= 1000000000000;
static long long best_fcall_time= 1000000000000;
static long long best_cas_time=1000000000000;
static long long best_mutex_time= 1000000000000;
static long long best_rwlock_time= 1000000000000;
static long long best_ft_time= 1000000000000;
static long long best_prelocked_time= 1000000000000;
//static long long best_cv_fair_rwlock_time= 1000000000000;; // fair from condition variables
//static long long best_fair_rwlock_time= 1000000000000;;
static long long mind(long long a, long long b) { if (a<b) return a; else return b; }
...
static int fcall_nop_2 (int i) __attribute__((__noinline__));
static int fcall_nop_2 (int i) {
    return i;
}

static void time_fcall (void) __attribute((__noinline__));
static void time_fcall (void) {
    struct timeval start,end;
    for (int t=0; t<T; t++) {
	gettimeofday(&start, NULL);
	for (int i=0; i<N; i++) {
...
static void time_cas (void) __attribute__((__noinline__));
static void time_cas (void) {
...
static void time_pthread_mutex (void) __attribute__((__noinline__));
static void time_pthread_mutex (void) {
#if 0 /* wkj: we don't care. toku_mutex uses "pthreads" internally. we
       * just want to make sure our ftfs code works */
...
#if 1
static void time_pthread_rwlock (void) __attribute__((__noinline__));
static void time_pthread_rwlock (void) {
...
void time_ft_rwlock (void) {
    ...
    memset(&external_mutex, 0, sizeof(external_mutex));
...
extern "C" int test_fair_rwlock(void);
int test_fair_rwlock () {
    if (timing_only) {
        if (1) { // to make it easy to only time the templated frwlock
            time_nop(); // OK
            time_fcall(); // OK
            time_cas(); // OK
            time_pthread_mutex(); // OK
            time_pthread_rwlock(); // OK
            time_ft_rwlock(); // OK
            time_ft_prelocked_rwlock(); // OK
	...
	if (verbose>0) {
            if (1) {
                ...
                printf("//  Best nop              time=%lldns\n", best_nop_time);
                printf("//  Best fcall            time=%lldns\n", best_fcall_time);
                printf("//  Best cas              time=%lldns\n", best_cas_time);
                printf("//  Best mutex            time=%lldns\n", best_mutex_time);
                printf("//  Best rwlock           time=%lldns\n", best_rwlock_time);
                printf("// WARNING: rwlock is not compared fairly please adjust the loops once the performance issue of nested lock is fixed\n");
                printf("//  Best ft rwlock        time=%lldns\n", best_ft_time);
                printf("//  Best prelocked        time=%lldns\n", best_prelocked_time);
                //printf("//  Best fair cv rwlock   time=%lldns\n", best_cv_fair_rwlock_time);
               // printf("//  Best fair fast rwlock time=%lldns\n", best_fair_rwlock_time);
            }
}
    }
    return 0;
