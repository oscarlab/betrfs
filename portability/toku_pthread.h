...
toku_mutex_init(toku_mutex_t *mutex, const toku_pthread_mutexattr_t *attr) {
    int r = pthread_mutex_init(&mutex->pmutex.pmutex, attr);
    assert_zero(r);
#if TOKU_PTHREAD_DEBUG
    mutex->locked = false;
...
extern toku_mutex_t checkpoint_safe_mutex;
...
#endif
    int r = pthread_mutex_destroy(&mutex->pmutex.pmutex);
    assert_zero(r);
}

static inline void
toku_mutex_lock(toku_mutex_t *mutex) {
    int r = pthread_mutex_lock(&mutex->pmutex.pmutex);
    assert_zero(r);
#if TOKU_PTHREAD_DEBUG
    invariant(mutex->valid);
    invariant(!mutex->locked);
...
toku_mutex_unlock(toku_mutex_t *mutex) {
    ...
    mutex->owner = 0;
#endif
    int r = pthread_mutex_unlock(&mutex->pmutex.pmutex);
    assert_zero(r);
}
...
toku_cond_init(toku_cond_t *cond, const toku_pthread_condattr_t *attr) {
    int r = pthread_cond_init(&cond->pcond.pcond, attr);
    assert_zero(r);
}

static inline void
toku_cond_destroy(toku_cond_t *cond) {
    int r = pthread_cond_destroy(&cond->pcond.pcond);
    assert_zero(r);
}
...
toku_cond_wait(toku_cond_t *cond, toku_mutex_t *mutex) {
    ...
    mutex->owner = 0;
#endif
    int r = pthread_cond_wait(&cond->pcond.pcond, &mutex->pmutex.pmutex);
    assert_zero(r);
#if TOKU_PTHREAD_DEBUG
    invariant(!mutex->locked);
    mutex->locked = true;
...
toku_cond_timedwait(toku_cond_t *cond, toku_mutex_t *mutex, toku_timespec_t *wakeup_at) {
    ...
    int r = pthread_cond_timedwait(&cond->pcond.pcond, &mutex->pmutex.pmutex, wakeup_at);
#if TOKU_PTHREAD_DEBUG
    invariant(!mutex->locked);
    mutex->locked = true;
    //mutex->owner = pthread_self();
...
toku_cond_signal(toku_cond_t *cond) {
    int r = pthread_cond_signal(&cond->pcond.pcond);
    assert_zero(r);
}

static inline void
toku_cond_broadcast(toku_cond_t *cond) {
    int r =pthread_cond_broadcast(&cond->pcond.pcond);
    assert_zero(r);
}
...
toku_pthread_rwlock_rdunlock(toku_pthread_rwlock_t *rwlock) {
    int r = pthread_rwlock_rdunlock(&rwlock->prwlock);
    assert_zero(r);
}
...
static inline void
toku_pthread_exit(void *return_ptr) {
    return pthread_exit(return_ptr);
}
