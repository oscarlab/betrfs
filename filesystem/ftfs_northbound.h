...
static inline char *nb_key_path(char *key)
{
	return (key);
}
...
#define SIZEOF_META_KEY(key) (strlen(nb_key_path(key)) + 1)
...
{
	return !strcmp(nb_key_path(key1), nb_key_path(key2));
}

static inline int
key_is_in_subtree_of_prefix(char *key, char *prefix, uint64_t prefix_size)
{
	return !memcmp(nb_key_path(key), nb_key_path(prefix), prefix_size - 1)
...
#define FTFS_BSTORE_MOVE_LARGE_THRESHOLD (2 << 21)

static inline enum ftfs_bstore_move_type
nb_bstore_get_move_type(struct ftfs_metadata *meta)
{
	if (S_ISDIR(meta->st.st_mode))
		return FTFS_BSTORE_MOVE_DIR;
	if (meta->st.st_size < FTFS_BSTORE_MOVE_LARGE_THRESHOLD)
