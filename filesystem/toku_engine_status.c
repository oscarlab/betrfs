...
static ssize_t toku_read_engine_proc(struct file *file, char __user *buf,
...
const struct file_operations toku_proc_engine_fops = {
	.read = toku_read_engine_proc,
	.write = toku_write_engine_proc,
};
...
void toku_engine_status_exit(void) {
	remove_proc_entry(TOKU_ENGINE_STATUS_PROC, NULL);
