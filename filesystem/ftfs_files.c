...
typedef int (*do_symlinkat_t)(const char*, int, const char *);
typedef long (*do_linkat_t)(int, const char*, int, const char*, int);
...
DECLARE_SYMBOL_FTFS(do_symlinkat);
DECLARE_SYMBOL_FTFS(do_linkat);
...
int resolve_ftfs_files_symbols(void)
{
	...
	LOOKUP_SYMBOL_FTFS(do_symlinkat);
	LOOKUP_SYMBOL_FTFS(do_linkat);
	return 0;
}
...
{
	unsigned long v = __ftfs_fdget(fd);
...
retry_deleg:
    ...
    ihold(inode);
    err = vfs_unlink(path.dentry->d_inode, dentry, &delegated_inode);
exit:
    dput(dentry);
...
slashes:
    err = !dentry->d_inode ?
          -ENOENT : S_ISDIR(dentry->d_inode->i_mode) ? -EISDIR : -ENOTDIR;
    goto exit;
}
...
int symlink(const char * oldname, const char * newname)
{
	...
	saved = get_fs();
	set_fs(get_ds());
	ret = ftfs_do_symlinkat(oldname, AT_FDCWD, newname);
	set_fs(saved);
	SOUTHBOUND_RESTORE;
...
int link(const char *oldpath, const char *newpath)
{
	...
	SOUTHBOUND_ATTACH;
	err = ftfs_do_linkat(AT_FDCWD, oldpath, AT_FDCWD, newpath, 0);
	SOUTHBOUND_RESTORE;
