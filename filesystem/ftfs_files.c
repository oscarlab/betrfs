...
/* 
 * this is a temporary hack. we should instead change the kernel and
 * add a helper that takes a files_struct */
...
/*
 * so that anyone who hasn't yet switched to the new kernel can
...
retry:
	...
	//is this taken care of in kern_path_locked?
	//it is checked in link_path_walk...
...
int fsync(int fd) {
	...
	if (f.file) {
		ret = vfs_fsync(f.file, 0); // fine in NB context
		ftfs_fdput(f);
	}
...
/**
 * TODO: perhaps all of the FILE * operations would be faster if we stored a
...
int fclose(FILE * stream) {
	...
	if (stream->rpos < stream->rend)
		fseek(stream, stream->rpos-stream->rend, SEEK_CUR);
...
int __toread(FILE *f)
{
	/* writeback your buffer updates, if any */
	if (f->wpos > f->buf)
		f->write(f, 0, 0);
...
/*
 *  this is only used by unit tests... do not waste time optimizing this.
 */
int fcopy(const char * src, const char * dest) {
	...
	out_error:
	saved_error = ftfs_get_errno();
	close(fd_src);
	if(fd_dest > 0)
		close(fd_dest);
	ftfs_set_errno(saved_error);
	return -1;
}

int utimensat(int dfd, const char *filename, const struct timespec *utimes,
	      int flags)
{
	int ret;
	...
	SOUTHBOUND_VARS;
	if (utimes) {
		memcpy(&tstimes, utimes, sizeof(tstimes));
		if (tstimes[0].tv_nsec == UTIME_OMIT &&
		    tstimes[1].tv_nsec == UTIME_OMIT)
			return 0;
	}
	SOUTHBOUND_ATTACH;
	ret = ftfs_do_utimes(dfd, filename, utimes ? tstimes : NULL,
			     flags);
	SOUTHBOUND_RESTORE;
	return ret;
}
