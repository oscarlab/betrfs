...
static ssize_t toku_read_flusher_proc(struct file *file, char __user *buf,
				     size_t count, loff_t *off)
{
	return 0;
}

static ssize_t toku_write_flusher_proc(struct file *file,
...
const struct file_operations toku_proc_flusher_fops = {
	.read = toku_read_flusher_proc,
	.write = toku_write_flusher_proc,
};

int toku_flusher_init(void) {
