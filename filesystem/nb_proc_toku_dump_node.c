...
static ssize_t toku_read_dump_node_proc(struct file *file, char __user *buf,
				     size_t count, loff_t *off)
{
	return 0;
}

static ssize_t toku_write_dump_node_proc(struct file *file,
				      ...
				      size_t count, loff_t *offp)
{
	...
	ret = count;
  	r = ftfs_bstore_dump_node(is_data, blocknum);
	if(r) {
...
const struct file_operations toku_proc_dump_node_fops = {
	.read = toku_read_dump_node_proc,
	.write = toku_write_dump_node_proc,
};

int toku_dump_node_init(void) {
