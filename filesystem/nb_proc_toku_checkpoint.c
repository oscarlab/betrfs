...
static ssize_t toku_read_checkpoint_proc(struct file *file, char __user *buf,
				     size_t count, loff_t *off)
{
	return 0;
}

static ssize_t toku_write_checkpoint_proc(struct file *file,
...
const struct file_operations toku_proc_checkpoint_fops = {
	.read = toku_read_checkpoint_proc,
	.write = toku_write_checkpoint_proc,
};

int toku_checkpoint_init(void) {
...
void toku_checkpoint_exit(void) {
	remove_proc_entry(TOKU_CHECKPOINT_PROC, NULL);
